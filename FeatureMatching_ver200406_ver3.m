...
function MORPHED_IMAGE = renderingLF_morph(posx, posy, view_near, view_far, windowsize, feature_term)
    ...
    for idx_w=1:size(ww, 2)
        ...
        for idx_h=1:size(hh, 2)
            h = hh(1, idx_h);
            w_n = round(w) + windowsize/2;
            h_n = round(h) + windowsize/2;

           im_far_sub = im_far((h_n-windowsize/2):(h_n+windowsize/2), (w_n-windowsize/2):(w_n+windowsize/2));
...
%            pause;
        ...
        for idx_h=1:2*height
           ...
           im_near_sub = im_near((h_n-windowsize/2):(h_n+windowsize/2), (w_n-windowsize/2):(w_n+windowsize/2));
