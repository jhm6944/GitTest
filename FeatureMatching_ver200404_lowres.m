...
function MORPHED_IMAGE = renderingLF_morph(LF_A0, LF_A1, POS_X, POS_Y)%, MatchedFeatures)
    ...
    for idx_w=1:size(ww, 2)
        ...
        im_near = rgb2gray(LoadFrame(LF_A0, P_near));
        im_far   = rgb2gray(LoadFrame(LF_A1, P_far));
...
%                 [val, loc] = min(cost_t2);
            ...
            elseif(farY > height)
                ...
                cost_t = cost_buff(1:count, :);
                cost_t2 = cost_t(:, 1) + cost_t(:, 2) / 50 + 250 * cost_t(:, 3);
                [val, loc] = min(cost_t2);
%                 cost_buff = zeros(bound, 3);
%                 count = 0;
%                 for idx_h_near = max(loc - bound_sub, 1):min(loc + bound_sub, bound)
...
% disp([num2str(near_idx) ', ' num2str(far_idx) ', ' num2str(cost_i) ', ' num2str(cost_g) ', ' num2str(cost_f)]);
% subplot(1, 2, 1);
% imshow(im_near_sub);
% subplot(1, 2, 2);
% imshow(im_far_sub);
