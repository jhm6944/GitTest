...
cellSize = [window/2 window/2];
im_near_sub = imgLookAt(im2double(im_near_360), 0, 0, window, fov);
nearDescriptor = extractHOGFeatures(im_near_sub, 'CellSize', cellSize);

featureSize = size(nearDescriptor, 2);
...
for x=1:width
    ...
    for y=1:height
        ...
        nearDescriptors(:, y) = extractHOGFeatures(im_near_sub, 'CellSize', cellSize);        
        
        im_far_sub = imgLookAt(im2double(im_far_360), LON(y, x), LAT(y, x), window, fov);
        farDescriptors(:, y) = extractHOGFeatures(im_far_sub, 'CellSize', cellSize);
    end
